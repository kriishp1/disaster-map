...
const API_key_Map = "YOUR_API_KEY_HERE";      // Enter your API_KEY

const map = new maplibregl.Map({        // Sets up map
    container: 'map',
    style: `https://api.maptiler.com/maps/darkmatter/style.json?key=${API_key_Map}`, // Enter your API_KEY 
    center: [-100, 40], // [lon, lat]
    zoom: 4,
    projection: 'globe' 
});

document.getElementById('donate').addEventListener('click', () => { // Link to goFundMe
...
document.getElementById("searchBtn").addEventListener("click", function () {
    ...
    if (year >= 1980 && year <= 2025) {     // Makes sure its a valid year
...
async function disasters(data, nameOfDisaster, yearSearchData) {
    ...
    if (disaster.length === 0) {
        ...
        const NoDataPopup = new maplibregl.Popup({ // Info on each Disaster
            closeButton: true,
            closeOnClick: true
        })
    ...
    disaster.forEach(event => {
        ...
        if (nameOfDisaster == "Wildfires") {                                    // Icon handling for each disaster
